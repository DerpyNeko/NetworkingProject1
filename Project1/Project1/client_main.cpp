...
int main(int argc, char **argv)
{
	...
	Buffer* buff = new Buffer(DEFAULT_BUFLEN);
	...
	do {
		...
		buff->WriteStringLE(input);

		// Step #4 Send the message to the server
		result = send(connectSocket, buff->ReadStringLE().c_str(), (int)strlen(input), 0);
		if (result == SOCKET_ERROR)
		{
		...
		buff->WriteStringLE(0, recvbuf);

		if (result > 0)
		{
			printf("Bytes received: %d\n", result);
			printf("Message: %s\n", buff->ReadStringLE(0).c_str());
		}
		else if (result == 0)
		{
